...
class Whatsapp::Providers::UnoapiService < Whatsapp::Providers::WhatsappCloudService
  ...
  def reset_group_invite_link(group_id)
    HTTParty.post("#{unoapi_group_path(group_id)}/invite_link/reset", headers: api_headers)
  end
  ...
  def unoapi_phone_path
    "#{api_base_path}/v15.0/#{whatsapp_channel.provider_config['business_account_id']}"
  end
end
